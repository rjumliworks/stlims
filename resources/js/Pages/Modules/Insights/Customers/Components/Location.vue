            ...
            <div class="table-responsive table-card">
                <simplebar data-simplebar style="height: 270px;">
                    <table class="table align-middle table-centered table-nowrap mb-3">
                        ...
                        <tbody class="fs-12">
                            <tr v-for="(list,index) in location" v-bind:key="index">
                                <td>{{index + 1}}</td>
                                <td>{{list.name}}</td>
                                <td class="text-center">{{list.address_count}} </td>
                                <td class="text-center">{{percentage(list.address_count)}}</td>
                            </tr>
                        </tbody>
                    </table>
                </simplebar>
...
export default {
    ...
    methods: {
        ...
        openView(type){
            window.open('/insights?option=customers-excel');
        },
    }
}
</script>