            ...
            <div class="table-responsive table-card">
                <simplebar data-simplebar style="height: 270px;">
                    <table class="table align-middle table-centered table-nowrap mb-3">
                        ...
                        <tbody class="fs-12">
                            <tr v-for="(list,index) in genders" v-bind:key="index">
                                <td>{{index + 1}}</td>
                                <td>{{list.name}}</td>
                                <td class="text-center">{{list.customer_gender_count}} </td>
                                <td class="text-center">{{percentage(list.customer_gender_count)}}</td>
                            </tr>
                        </tbody>
                    </table>
                </simplebar>
...
export default {
    ...
    computed: {
        total() {
            return this.genders.reduce((sum, item) => sum + item.customer_gender_count, 0);
        }
    },
    methods: {
        percentage(data){
