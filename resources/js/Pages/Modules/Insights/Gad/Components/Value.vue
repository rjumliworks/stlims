        ...
        <div class="card-header bg-light-subtle">
            <div class="d-flex mb-n3">
                ...
                <div class="flex-grow-1">
                    <h5 class="mb-0 fs-14"><span class="text-body">Value of Transaction</span></h5>
            ...
            <div class="table-responsive table-card">
                <simplebar data-simplebar style="height: 270px;">
                    <table class="table align-middle table-centered table-nowrap mb-3">
                        ...
                        <tbody class="fs-12">
                            <tr v-for="(list,index) in values" v-bind:key="index">
                                <td>{{index + 1}}</td>
                                <td>{{list.name}}</td>
                                <td class="text-center">{{percentage(list.count)}}</td>
                                <td class="text-center">{{formatMoney(list.count)}} </td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr>
...
export default {
    ...
    computed: {
        total() {
            return this.values.reduce((sum, item) => sum + Number(item.count), 0);
        }
    },
    methods: {
        percentage(data){
