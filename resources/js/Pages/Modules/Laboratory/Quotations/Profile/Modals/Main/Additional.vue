<template>
    <b-modal v-if="selected" v-model="showModal" header-class="p-3 bg-light" :title="(!editable) ? 'Add additional fee' : 'Update additional fee'" class="v-modal-custom" modal-class="zoomIn" centered no-close-on-backdrop>
        ...
        <template v-slot:footer>
            ...
            <b-button @click="submit('ok')" variant="primary" :disabled="form.processing" block>{{ (!editable) ? 'Submit' : 'Update' }}</b-button>
        </template>
    </b-modal>
</template>
<script>
...
export default {
    ...
    methods: { 
        ...
        edit(data,id){
            this.selected = data;
            this.selected.name = data.service.name;
            this.form.q_id = id;
            this.form.id = data.id;
            this.form.option = 'additional';
