...
class PezaExport implements FromView
{
    ...
    {
         $lists = Customer::select('customers.id', 'customers.name','customers.is_main', 'customer_names.name as customer_name','customer_names.has_branches as has_branches',\DB::raw('COUNT(tsrs.id) as count'),\DB::raw('SUM(tsr_payments.total) as total'))
