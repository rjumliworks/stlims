...
class AnalystClass
{
    ...
    public function laboratories(){
        $data = UserRole::with('laboratory:id,name')->where('user_id',\Auth::user()->id)->whereIn('role_id',[4,9])->get()->map(function ($item) {
            return [
    ...
    public function lists($request){
        ...
        return [
            [
                ...
                'count' => TsrAnalysis::where('status_id',11)->where('analyst_id',\Auth::user()->id)->whereYear('start_at',now())->count(),
            ...
            [
                ...
                'description' => 'Tests conducted by the analyst',
                'count' => TsrAnalysis::where('status_id',12)->where('analyst_id',\Auth::user()->id)->whereYear('start_at',now())->count(),
                'icon' => 'ri-checkbox-circle-fill fs-20',
                'color' => 'text-success'
            ],
            [
                ...
                'count' => '₱' . number_format(TsrAnalysis::where('status_id',12)->where('analyst_id',\Auth::user()->id)->whereYear('start_at',now())->sum('fee'),2),
